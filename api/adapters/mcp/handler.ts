...
export async function serveMcp(request:IncomingMessage,response:ServerResponse,body:unknown,agents:AgentModule,agent:AuthenticatedAgent) {
  const handler=createMcpHandler(()=>{
    ...
    register('dig_get_workflow','Read Columns, flow roles, limits, workflow revision, and agent work settings including the configured review destination.',{ spaceKey },input=>({ kind:'workflow',...input }))
    if(agent.scope==='tasks:work') {
      ...
      write('dig_add_comment','Add a relevant progress comment to a claimed Task. No heartbeat comments. Use the dedicated blocker or review tool for handoff.',{...claimed,text:z.string().min(1).max(5000),mentions:z.array(z.uuid().transform(value=>value as MemberId)).max(25).default([])},({text,mentions,...input})=>({kind:'change',...input,command:{kind:'add-comment',taskId:input.taskId,text,mentions}}))
      const mentions=z.array(z.uuid().transform(value=>value as MemberId)).max(25).default([])
      write('dig_report_blocker','Report what blocks this Task and what help is needed. Atomically posts the report, notifies the delegating person, and releases this claim. Leaves the Column unchanged. Retry lost responses with the same request ID.',{...versioned,report:z.object({summary:z.string().min(1).max(agentReportLimits.summary),needed:z.string().min(1).max(agentReportLimits.needed)}).strict(),mentions},({report,mentions,expectedRevision,...input})=>({kind:'change',...input,command:{kind:'report-blocker',task:{taskId:input.taskId,expectedRevision},report,mentions}}))
      write('dig_handoff_review','Hand this Task to a human for review. Read the current workflow first and include its revision. Atomically posts your report, moves to its configured review Column, notifies the delegating person, and releases the claim. Report failed or unrun checks honestly; this does not close or certify the Task. Retry lost responses with the same request ID.',{...versioned,expectedWorkflowRevision:z.number().int().positive().transform(value=>value as Revision),report:z.object({summary:z.string().min(1).max(agentReportLimits.summary),verification:z.object({outcome:z.enum(['passed','failed','not-run']),details:z.string().min(1).max(agentReportLimits.verification)}).strict(),limitations:z.string().min(1).max(agentReportLimits.limitations),reference:z.string().min(1).max(agentReportLimits.reference).optional()}).strict(),mentions},({report,mentions,expectedRevision,expectedWorkflowRevision,...input})=>({kind:'change',...input,command:{kind:'handoff-review',task:{taskId:input.taskId,expectedRevision},expectedWorkflowRevision,report,mentions}}))
      ...
      write('dig_move_task','Move claimed work to a non-Completion Column as an ordinary progress update. Respect WIP warnings. The configured review destination requires dig_handoff_review.',{...versioned,columnId:z.uuid().transform(value=>value as ColumnId),expectedOrderRevision:z.number().int().positive().transform(value=>value as Revision),place},({expectedRevision,columnId,expectedOrderRevision,place,...input})=>({kind:'change',...input,command:{kind:'place-task',task:{taskId:input.taskId,expectedRevision},destination:{columnId,expectedOrderRevision,place}}}))
    }
    return server
  },{ legacy:'stateless' })
