...
run('Slice 4 browser through HTTP and PostgreSQL', () => {
  ...
  it('moves a Task by keyboard through HTTP and persists its Column', async () => {
    ...
    const server = createTeamServer({ agents: new AgentModuleImplementation(db,new BoardModuleImplementation(db)), exports: new SpaceExportModuleImplementation(db), identity, space, board }, loadConfig({ ALLOWED_ORIGINS: origin }))
    ...
    try {
      ...
      await select('Column', overview.columns[1].id)
      await waitFor(() => expect(screen.getByLabelText('Column')).toBeEnabled())
      ...
      await user.click(screen.getByRole('button', { name: /DIG-1/ }))
      await user.click(await screen.findByLabelText('More Task actions'))
      ...
      await select('Column', overview.columns[0].id)
