---
name: dig
description: Read or search Tasks in Dig through its configured MCP connection, including requests such as "show DIG-42". Use when the user refers to work in Dig or invokes $dig.
---

# Dig

This release supports read access. The user connects an external Codex session when needed; no Board watcher or background agent is required.
...
A bare key or `$dig show DIG-42` means read and explain. For `$dig work DIG-42`, read the context and explain that claims and Task updates require a later Dig release. A Task does not imply permission to push code or deploy.
