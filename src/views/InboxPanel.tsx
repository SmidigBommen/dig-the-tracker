...
export function InboxPanel({ session, state, readOnly }: { session: BoardSession; state: BoardSessionState; readOnly: boolean }) {
  const reasons = { assignment: 'assigned you', mention: 'mentioned you', comment: 'commented on your Task', 'agent-blocked':'reported a blocker', 'agent-review':'requested your review' }
  return <section className="notification-inbox" aria-label="Notifications">
    ...
    <ol>{state.inbox?.items.map((notification) => <li key={notification.id}>
      <p>{notification.actor.displayName}{notification.agent ? ` via ${notification.agent.connectionName}` : ''} {reasons[notification.kind]}{!notification.read && <span className="unread-notification">Unread</span>}</p>
