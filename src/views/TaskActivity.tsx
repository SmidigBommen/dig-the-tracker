...
import type { Outcome, TaskHistoryEntry, TaskReference } from '../../api/contracts/board.ts'
      ...
      {tab === 'Comments' ? <CommentPanel session={session} state={state} disabled={disabled} />
        : <div className="task-history">
          {!history ? <Button disabled={state.busy} onClick={() => void session.loadHistory()}>{state.busy ? 'Loading history…' : 'Load history'}</Button>
            : <><ol>{history.items.map((entry) => <HistoryEntry key={entry.id} session={session} references={state.references} entry={entry} />)}</ol>
...
function HistoryEntry({ entry,session,references }: { entry: TaskHistoryEntry; session: BoardSession; references?: TaskReference[] }) {
  return <li><p><strong>{entry.actor.displayName}</strong> {entry.summary}
    ...
    {entry.comment && <PlainText session={session} references={references} text={entry.comment} />}</li>
}
