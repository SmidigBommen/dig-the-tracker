...
`AgentModule` exposes `manage`, `authenticate`, `read`, and `work`. Browser management requires an opaque human identity and live session; MCP authenticates a named, expiring connection into a separate opaque capability. The Agent Module owns connection, grant, and run SQL. Private Space helpers own policy and membership reads, and a private Identity helper owns browser-session checks.

MCP translates six read tools into these operations. Task reads reuse the Board Module with a privately constructed Space capability containing an agent credential instead of a browser session. The Board transaction locks Space, Member, then connection and rechecks current membership generation, Space policy, connection revision, expiry, and selected grants. Only an explicitly work-scoped connection with a valid private run credential can enter a restricted Board change path. Agent capabilities cannot enter follow, inbox, analytics reports, or export behavior.

An administrator must enable each Space. Connections select explicit enabled Spaces and default to read-only scope. Work consent creates a separate `tasks:work` connection; token replacement preserves permission. Agent absence has no effect on human use. See [Slice 11 implementation](docs/implementation/slice-11-agent-read-access.md) for limits, client evidence, and remaining release gates; [Slice 12 implementation](docs/implementation/slice-12-agent-work.md) describes claims and protected writes.

Board owns Task claims and claim invalidation. Board serialization guarantees one claim per Task; existing Subtasks need separate claims, and creation plus claim is atomic. Agent runs bind connection revision, membership generation, Space policy, and a Space lifecycle epoch. Run keys use domain-separated HMAC under SESSION_SECRET, remain private to the starting session, and never enter Task projections or history. Public run IDs provide attribution only. Rotating SESSION_SECRET invalidates run keys.

Writes lock Space, Member, connection, then Board. Current access is rechecked before a namespaced receipt lookup; committed retries survive claim release/expiry. New changes then require the live run and matching Task claim. Task changes use revision checks and delegated history. Human assignment, closure, and archive invalidate claims through a Board-table trigger, including scheduled/family operations. Revocation, token replacement, and policy disablement publish Board updates so connected browsers refresh claims. Reads filter invalid claims and the UI expires badges at the earlier claim or token deadline. No worker, timer job, heartbeat comment, or connected client is needed to expire authority.

Board also owns agent work settings and the stable review Column relationship. Work requires an enabled mapping to an existing non-Completion Column. Workflow edits atomically preserve a valid mapping or disable work and delete claims. Read access remains independent. The Slice 13 migration pauses existing work and ends temporary claims until an administrator chooses the review destination.

Blocker and review commands require the current Task revision and claim. Review also checks the workflow revision and uses the current mapping under the Board lock. It posts a marked Comment, performs any review move, notifies the delegating Member, deletes the claim, and appends history and a receipt in one transaction. Blockers leave the Column unchanged. Ordinary agent movement into the review destination returns `review-handoff-required`. Reports include explicit passed, failed, or not-run verification; they never close or certify a Task.

Ordinary human self-notifications remain suppressed. Agent report notifications allow the delegating Member as both actor and recipient, with required agent attribution. Explicit mentions still notify other Members. Reports use existing editable Comments and tombstones, with an origin label and edited indicator; they are not immutable verification certificates. See [Slice 13 notes](docs/implementation/slice-13-agent-handoff.md) for failure and deployment behavior.
