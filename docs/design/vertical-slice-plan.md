...
Status: Slices 1 through 4 implemented. Slices 5 through 9 remain planned.
...
Slice 4 is implemented. See [movement and flow notes](../implementation/slice-04-move-work-and-record-flow.md). Start Slice 5 with comments through `BoardModule.change` and independent comment pages. Preserve Task/order revisions, request receipts, transaction-time access checks, atomic events and Board updates, and uncertain browser-request retries. Connect closing comments already stored in closure history to the comment model before adding mentions and Notifications. Board-owned unassignment already runs in the Space membership transaction.
