...
Status: Slices 1 through 6 implemented. Slices 7 through 9 remain planned.
...
Slice 6 is implemented. See [live collaboration notes](../implementation/slice-06-collaborate-live.md). Start Slice 7 with declarative workflow changes, scheduled Task archive, and Space deletion. Preserve the live feed, bounded snapshots, recipient privacy, draft recovery, and established lock order. Workflow and lifecycle changes must be visible to already-open browsers.
