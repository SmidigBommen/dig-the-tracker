...
Status: Slices 1 and 2 implemented. Slices 3 through 9 remain planned.
...
Start Slice 3 with Task capture through `BoardModule.change`, the HTTP Adapter, and the browser `BoardSession`. Before assignment becomes reachable, add the Board-owned operation that atomically unassigns a removed Member's open Tasks inside the Space membership transaction. Add one Task creation outcome first, then grow detail, edit, assignment, tags, bounded pages, archive, and restore through separate red-green cycles.
