...
Status: Slices 1 through 5 implemented. Slices 6 through 9 remain planned.
...
Slice 5 is implemented. See [discussion and Notification notes](../implementation/slice-05-discuss-and-notify.md). Start Slice 6 with `BoardModule.follow`, retained sequences, and the Server-Sent Events Adapter. Preserve recipient privacy, transaction-time access checks, bounded pages, atomic receipts and updates, and uncertain browser-request retries. Filter Member-specific read-state projections before delivery and reload the recipient inbox on invalidation. Board-owned unassignment already runs in the Space membership transaction.
