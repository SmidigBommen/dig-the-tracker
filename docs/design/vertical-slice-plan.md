...
| 9. Choose a personal theme. Complete | Each person chooses Nature, Neutral, or Tokyo Night and System, Light, or Dark mode; the whole tracker updates while retaining open work. | Add account-owned appearance preferences, a header Appearance chooser with labelled previews, six accessible palettes using shared colour tokens, initial theme resolution, and same-browser tab updates. See [the agreed scope](slice-09-personal-themes.md). | Passed: contrast and keyboard checks; all-screen and phone coverage; account isolation; persistence across reload, sign-in, Spaces, and devices; system-mode changes; tab synchronization; save-failure recovery; draft preservation. |
| 10. Release on the Hetzner server. Implementation complete; production gates pending | The same OCI image runs locally with Compose and in Coolify, exports a Space, survives graceful shutdown, exposes health state, and has a tested backup restore. | Delivered versioned JSON export, runtime logging and drain, backup/restore scripts, retention configuration, browser checks, and monitoring hooks. Existing checksummed migrations, locking, health, and container assembly are verified. | Local export, schema, privacy, migration, image/Compose, browser/axe, scale, and restore checks pass. Production rollout, off-server storage and restore, alerts/log retention, and manual accessibility/browser checks remain. See [Slice 10 evidence](../implementation/slice-10-release-readiness.md). |
...
## Deployment follow-up: 2026-09-08

The branch switch and auto-deploy checks are complete. Production browser verification remains open:
...
- [ ] Verify production browser sign-in and Task editing.
